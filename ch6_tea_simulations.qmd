...
# the distribution of the high_n data is narrower (closer to the mean) and more normally distributed. The low_n data has more spread.
```
...
# the distribution is uniform when the null is true
```


```{r}
sum(all_results$pvalue<.05)
# We would get false positives for 7 out of 100 experiments
# We would report a statistically significant effect, but it is just due to random noise
```
...
```
```{r}
sum(all_results_high_n$pvalue>.05)
# we would fail to reject 3 out of 100 experiments
```
